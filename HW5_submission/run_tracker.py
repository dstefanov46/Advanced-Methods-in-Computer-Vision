...
def evaluate_tracker(dataset_path, network_path, results_dir, visualize):
    ...
    for sequence_name in sequences:
        ...
        # this variable 'tracking' is used to tell the program if we're in tracking mode or re-detection mode
        tracking = True
        for i in range(1, sequence.length()):
            ...
            if tracking == True:
                ...
                if score > 5:     # threshold for car9
                    ...
                    results.append(prediction)
                    scores.append([0])            # if we think target not in our patch, we report conf score 0

            # if we're not tracking, then we enter the re-detection process
            else:
                prediction, score = tracker.redetect(img)
                if score < 5:
                    prediction = np.zeros(4)
                    results.append(prediction)
                    scores.append([0])            # if we couldn't re-detect the target, we report conf score 0
                else:
                    tracking = True
                    tracker.center = np.array([prediction[1] - 1 + (tracker.target_sz[0] - 1) / 2,
                                               prediction[0] - 1 + (tracker.target_sz[1] - 1) / 2])
                    results.append(prediction)
                    scores.append([score])

            if visualize:

                # we print current frame in sequence
