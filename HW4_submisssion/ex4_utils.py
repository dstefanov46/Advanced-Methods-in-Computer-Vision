...
# Added get_patch function from Project 2
...
# Added extract_histogram function from Project 2
...
# Added create_epanechnik_kernel function from Project 2
...
# Sometimes happens that there's one more row or column in the patch matrix than in the kernel matrix, and vice versa,
# so with this function, we make both matrices the same size
def get_equal_sizes(patch, kernel):
    height = min(patch.shape[0], kernel.shape[0])
    width = min(patch.shape[1], kernel.shape[1])
